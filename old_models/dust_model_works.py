...
if __name__ == '__main__':
    ...
    #Parallel(n_jobs=num_cores)(delayed(get_scalob)(c) for c in good_world)

    # create a file to store data
    with h5py.File(os.path.join(data_loc, 'new_data_temp.h5'), 'w') as hf:
        z = len(str(len(y_fuv)))

        # args is what is passed to lnprob in addiiton to theta
        # this loops over one pixel at a time
        inds = range(150,1000)  #range(len(y_fuv))
        for i in inds:
            t0 = time.time()
...
## To-Do
## 1. Record output using hdf5 (save all chains and metadata)
## 2. Include A_V as a parameter : adjust priors such that av prior is a guassian centered at MATCH value with very small dispersion (.05 mag)
## 3. Flexibility to loop over n points
## 4. Model distribution of rvs
